...
def imu_publisher(sock):
	...
	mag_pub = rospy.Publisher('imu/mag', Vector3Stamped, queue_size=3)
	acl_pub = rospy.Publisher('imu/accel', Vector3Stamped, queue_size=3)
	gyro_pub = rospy.Publisher('imu/gyro', Vector3Stamped, queue_size=3)
	imu_pub = rospy.Publisher('imu', Imu, queue_size=3.3)
	...
	while not rospy.is_shutdown():
	    ...
	    #          rospy.loginfo('x %s y %s z %s', gyro_x, gyro_y, gyro_z)
	       ...
	       acl_msg.vector.x = float(line[0])
	       acl_msg.vector.y = float(line[1])
	       acl_msg.vector.z = float(line[2])
